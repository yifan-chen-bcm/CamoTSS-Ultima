...
### Timestamped progress logging. Wall clock + elapsed since import.
### flush=True matters: stdout is block-buffered when redirected to a file,
### so without it nothing appears until the process exits.
_T0 = time.time()


def _log(msg):
    el = int(time.time() - _T0)
    print("[%s | +%02d:%02d:%02d] %s" % (time.strftime('%H:%M:%S'),
                                         el // 3600, (el % 3600) // 60, el % 60, msg), flush=True)
...
class get_TSS_count():
    ...
    def __init__(self, generefPath, tssrefPath, bamfilePath, fastqFilePath, outdir, cellBarcodePath, nproc, minCount,
                 maxReadCount, clusterDistance, InnerDistance, windowSize, minCTSSCount, minFC):
    ...
    def _getreads(self, bamfilePath, fastqFilePath, geneid, mergedf):
        ...
        if mergedf.loc[geneid]['Strand'] == '+':
            reads1_umi = [r for r in reads1_umi if editdistance.eval(
                fastqFile.fetch(start=r.reference_start - 14, end=r.reference_start - 1,
                                region='chr' + str(mergedf.loc[geneid]['Chromosome'])), 'TTTCTTATATGGG') > 3]
        elif mergedf.loc[geneid]['Strand'] == '-':
            reads1_umi = [r for r in reads1_umi if editdistance.eval(
                fastqFile.fetch(start=r.reference_end, end=r.reference_end + 13,
                                region='chr' + str(mergedf.loc[geneid]['Chromosome'])), 'CCCATATAAGAAA') > 3]
        ...
        if mergedf.loc[geneid]['Strand'] == '+':
            ...
            readsdf = pd.DataFrame(
                {'name': [r.query_name for r in reads1_umi], 'TSS': [r.reference_start for r in reads1_umi],
                 'UMI': [r.get_tag('UB') for r in reads1_umi], 'CB': [r.get_tag('CB') for r in reads1_umi]})
            ...
            readsdf = pd.DataFrame(
                {'name': [r.query_name for r in reads1_umi], 'TSS': [r.reference_end - 1 for r in reads1_umi],
                 'UMI': [r.get_tag('UB') for r in reads1_umi], 'CB': [r.get_tag('CB') for r in reads1_umi]})
            readsdf.sort_values(['UMI', 'CB', 'TSS'], inplace=True)
    ...
    def _get_gene_reads(self):
        ...
        for read in getreadsFile.fetch(until_eof=True):
            nrec += 1
            if nrec == nextrep:
                _log("phase 1/6  %d M records scanned, %d genes seen" % (nrec // 1_000_000, len(geneidset)))
                nextrep += 25_000_000
            if read.has_tag('GX'):
                geneidset.add(read.get_tag('GX'))
        geneid_uniqdf = pd.DataFrame(sorted(geneidset), columns=['gene_id'])
        _log("phase 1/6  done: %d records, %d genes with reads" % (nrec, len(geneidset)))
        ...
        # store reads fetched
        _log("phase 2/6  %d genes kept after maxReadCount=%d trim; writing fetch_reads.pkl"
             % (len(readinfodict), self.maxReadCount))
    ...
    def _do_clustering(self, dictcontentls):
        ...
        clusterModel = AgglomerativeClustering(n_clusters=None, linkage='average',
                                               distance_threshold=self.InnerDistance)
        ...
        for i in range(0, len(finallabel)):
            altTSSls.append([posiarray[labels == finallabel[i]], CBarray[labels == finallabel[i]],
                             cigartuplearray[labels == finallabel[i]]])
    ...
    def _do_hierarchial_cluster(self):
        ...
        _log("phase 3/6  done: %d genes have at least one cluster (%d s)"
             % (len(altTSSdict), int(time.time() - start_time)))
    ...
    def _filter_false_positive(self):
        ...
        for i in altTSSdict.keys():
            for j in range(0, len(altTSSdict[i])):
                ...
                unencoded_G_percent = sum([('14S' in ele) or ('15S' in ele) or ('16S' in ele) for ele in
                                           altTSSdict[i][j][2].flatten()]) / count
        ...
        fourfeaturedf.columns = ['UMI_count', 'SD', 'summit_UMI_count', 'unencoded_G_percent', 'NO.TSS', 'gene_id',
                                 'summit_position']
        fourfeature_output = self.count_out_dir + 'fourFeature.csv'
        fourfeaturedf.to_csv(fourfeature_output)

        _log("phase 4/6  %d candidate clusters scored -> fourFeature.csv" % (len(fourfeaturedf),))
        ...
        if USE_LOGISTIC_FILTER:
            pathstr = str(
                Path(os.path.dirname(os.path.abspath(__file__))).parents[0]) + '/model/logistic_4feature_model.sav'
        ...
        afterfiltereddf.columns = ['UMI_count', 'SD', 'summit_UMI_count', 'unencoded_G_percent', 'NO.TSS', 'gene_id',
                                   'summit_position']

        afterfilter_output = self.count_out_dir + 'afterfiltered.csv'
        afterfiltereddf.to_csv(afterfilter_output)
        _log("phase 4/6  %d of %d clusters retained (logistic filter %s)"
             % (len(afterfiltereddf), len(fourfeaturedf),
                "ON" if USE_LOGISTIC_FILTER else "BYPASSED"))
    ...
    def _do_anno_and_filter(self, inputpar):
        ...
        for i in range(0, len(tssls)):
            if (tssls[i] >= np.min(altTSSitemdict[i][0])) & (tssls[i] <= np.max(altTSSitemdict[i][0])):
                ...
                transcriptdict[name1] = (altTSSitemdict[row_ind[i]][0], altTSSitemdict[row_ind[i]][1],
                                         altTSSitemdict[row_ind[i]][2])
            else:
                newname1 = str(geneid) + '_newTSS'
                transcriptdict[newname1] = (altTSSitemdict[row_ind[i]][0], altTSSitemdict[row_ind[i]][1],
                                            altTSSitemdict[row_ind[i]][2])
    ...
    def _TSS_annotation(self):
        ...
        _log("phase 5/6  matching clusters to annotated transcript TSSs, %d genes" % (len(inputpar),))
        with multiprocessing.Pool(self.nproc) as pool:
        ...
        d = {'transcript_id': [transcript[0] for transcript in extendls],
        ...
        _log("phase 5/6  done: %d annotated clusters (%d s)" % (len(extendls), int(time.time() - start_time)))
        # print(extendls)
        # print(regiondf)
    ...
    def produce_sclevel(self):
        _log("CamoTSS TC pipeline starting")
        ctime = time.time()
        ...
        _log("phase 6/6  building count matrix: %d cells x %d clusters (single-threaded)"
             % (len(cellIDset), len(extendls)))
        ...
        for i in range(0, len(extendls)):
            ...
            if (i + 1) % 1000 == 0 or i + 1 == len(extendls):
                _log("phase 6/6  %d/%d columns (%.1f%%)" % (i + 1, len(extendls), 100.0 * (i + 1) / len(extendls)))
        ...
        _log("phase 6/6  writing scTSS_count_all.h5ad (%d x %d)" % adata.shape)
        ...
        selectedf = newdf[
            newdf.duplicated('gene_id', keep=False)]  # get data frame which includes two transcript for one gene
        ...
        for i in geneID:
            ...
            keepdf = tempdf[tempdf['diff'].isna() | tempdf['diff'].abs().ge(
                self.clusterDistance)]  # want to get TSS whose cluster distance is more than user defined.
        ...
        _log("phase 6/6  writing scTSS_count_two.h5ad (%d x %d): genes with >=2 clusters"
             % finaltwoadata.shape)
        finaltwoadata.write(sc_output_h5ad)

        _log("all done: %d cells x %d clusters (%d clusters in multi-TSS genes)"
             % (adata.shape[0], adata.shape[1], finaltwoadata.shape[1]))
    ...
    def _get_CTSS(self, fetchadata):
        ...
        alloneclusterdf['TSS_start'] = \
        alloneclusterdf['Unnamed: 0'].str.split('*', expand=True)[1].str.split('_', expand=True)[0].astype('float')
    ...
    def produce_CTSS_adata(self):
        ...
        ctssvardf['counts_dropped_UnencodedG'] = \
        ctssvardf['clusterID'].str.split('@', expand=True)[1].str.split('$', expand=True)[0]
